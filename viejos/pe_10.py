...
# Updated logic to correctly parse filenames like "lstm_predictions_run_X_with_signals.csv"
existing_runs = glob.glob("lstm_predictions_run_*.csv")
if existing_runs:
    run_numbers = []
    for f in existing_runs:
        try:
            # Expected format: "lstm_predictions_run_X_with_signals.csv"
            # Split by '_' and get the part before '_with_signals'
            # Example: "lstm_predictions_run_1_with_signals.csv" -> ["lstm", "predictions", "run", "1", "with", "signals.csv"]
            # We want the element before "with"
            parts = f.split("_")
            if 'with' in parts: # Check if the new format is present
                # Find index of 'with' and get the element just before it
                run_num_str = parts[parts.index('with') - 1]
                run_numbers.append(int(run_num_str))
            else: # Fallback for old format: "lstm_predictions_run_X.csv"
    ...
    if run_numbers:
        ...
        run_number = 1 # If no valid run numbers found, start from 1
else:
